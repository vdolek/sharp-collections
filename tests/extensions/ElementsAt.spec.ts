...
describe('elementsAt tests', () => {
    ...
    it('elementsAtRest test', () => {
        const list = List.fromRest(0, 2, 4, 6, 8, 10, 12);

        expect(list.elementsAtRest().toArray()).to.be.deep.equal([]);
        expect(list.elementsAtRest(1, 4, 3).toArray()).to.be.deep.equal([2, 8, 6]);
        expect(() => list.elementsAtRest(1, 4, 99, 3).toArray()).throws(Error, 'Index was out of range');
        expect(() => list.elementsAtRest(1, 4, -99, 3).toArray()).throws(Error, 'Index was out of range');
    });
});
